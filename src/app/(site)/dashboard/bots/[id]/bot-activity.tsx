...
export function BotActivity({ messages, leads }: { messages: Msg[]; leads: LeadRow[] }) {
  ...
  return (
    <div className="mt-12 space-y-10 border-t border-gray-200 pt-12">
      <section>
          ...
          <ul className="mt-4 divide-y divide-gray-100 rounded-xl border border-gray-100 bg-white shadow-sm">
            {leads.map((l) => (
              <li key={l.id} className="flex flex-col gap-1 px-4 py-3 text-sm sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-2">
                <span className="break-all font-medium text-gray-900">{l.email}</span>
                <time className="text-gray-500" dateTime={l.createdAt.toISOString()}>
      ...
      <section>
        ...
        <p className="mt-1 text-sm text-gray-600">Last {messages.length} rows stored for this assistant.</p>
        {messages.length === 0 ? (
          <p className="mt-4 text-sm text-gray-500">No chat history yet.</p>
        ) : (
            ...
            {chronological.map((m) => (
              <li
                ...
                className={`rounded-xl border px-3 py-2 text-sm break-words ${
                  m.role === "user"
                    ? "ml-0 border-gray-200 bg-gray-100 text-gray-900 sm:ml-4"
                    : "mr-0 border-teal-100 bg-teal-50/50 text-gray-800 sm:mr-4"
                }`}
              >
